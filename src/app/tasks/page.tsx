...
import { collection, getDocs, doc, getDoc, Timestamp, updateDoc } from 'firebase/firestore';
...
export default function PMTasksPage() {
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      ...
      const uids = new Set(allTasks.map(t => t.assignedTo).filter(Boolean));
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="space-y-2">
        {filteredTasks.map(t => {
          const resp = t.assignedTo ? nameCache.get(t.assignedTo) || 'Cargando...' : 'Sin asignar';
          return (
            <div
              key={`${t.type}-${t.id}`}
            ...
            >
              <div>
                ...
                <div className="text-xs text-gray-500">Responsable: {resp}</div>
              </div>
              <div className="text-sm text-gray-600">{t.dueDate ? t.dueDate.toLocaleDateString() : 'Sin fecha'}</div>
            </div>
          );
        })}
        {filteredTasks.length === 0 && <div className="text-gray-500">No hay tareas para mostrar.</div>}
      </div>
    </div>
  );
}
